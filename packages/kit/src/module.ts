import type { MangiaConfig, MangiaHooks, MangiaModule, MangiaModuleContext, MangiaModuleMeta, MangiaPage } from '@mangia/schema'
...
function resolveModulePath(entry: string, rootDir: string, _require: NodeJS.Require): string {
  ...
  if (existsSync(resolved) && statSync(resolved).isDirectory()) {
    ...
    if (existsSync(pkg)) {
      ...
      if (exports?.['.']?.import)
        return resolve(resolved, exports['.'].import)
      if (exports?.['.']?.default)
        return resolve(resolved, exports['.'].default)
      if (main)
        return resolve(resolved, main)
    }
  }
  return resolved
}
